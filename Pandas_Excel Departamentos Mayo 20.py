...
def prom_pers_d():
    ...
    prom_p=total_p/len(n_dep) # len=longitud, promedio de poblacion, pro/total departamentos
    
    return prom_p

def prom_area_d():

    prom_d=area_col()/len(n_dep) # igual que promedio  pero con area
    return prom_d 

def numero_mayor():
    d_mayor=0
    for x in range (len(n_dep)):
        index_d= archivoExcel.loc[x,'Poblacion']  # departamento con mayor poblacion .loc es ubicar      
        if index_d>d_mayor:
            d_mayor=index_d
    return d_mayor
...
def numero_menor():
    ...
    for m in range (len(n_dep)):        
        index_d2= archivoExcel.loc[m,'Poblacion'] # menor poblacion .loc ubicar
        if index_d2<d_menor:
            d_menor=index_d2
...
#crear el gráfico
plt.bar(archivoExcel['Departamento'],archivoExcel['Poblacion']) # grafico barras
plt.show()
plt.barh(archivoExcel['Departamento'],archivoExcel['Poblacion']) # grafico barras horizontal
plt.show()
plt.pie(archivoExcel['Poblacion'],labels=archivoExcel['Departamento']) # grafico de pastel
plt.show() # ver grafico, imprimir grafica
